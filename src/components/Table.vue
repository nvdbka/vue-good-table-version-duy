		...
		<div class="vgt-inner-wrap" :class="{ 'is-loading': isLoading }">
			...
			<div :class="{ 'vgt-responsive': responsive }" :style="wrapperStyles">
				<table id="vgt-table" ref="table" :class="tableStyles">
					...
					<div class="table-body-area">
						<tbody v-for="(headerRow, hIndex) in paginated" :key="hIndex">
							...
							>
								...
								<td @click="onCellClicked(row, column, index, $event)" v-for="(column, i) in columns" :key="i" :class="getClasses(i, 'td', row)" v-if="!column.hidden && column.field" v-bind:data-label="compactMode ? column.label : undefined">
					...
					<div v-if="showEmptySlot" class="table-body-area">
...
export default {
	...
	methods: {
		...
		initializePagination() {
			const { enabled, perPage, position, perPageDropdown, perPageDropdownEnabled, dropdownAllowAll, firstLabel, lastLabel, nextLabel, prevLabel, rowsPerPageLabel, ofLabel, pageLabel, allLabel, setCurrentPage, mode, infoFn } = this.paginationOptions;
